...
public class ModItemModelProvider extends ItemModelProvider {
    ...
    public ModItemModelProvider(PackOutput output, ExistingFileHelper existingFileHelper) {
        super(output, CrusadersProgressionReimagined.MOD_ID, existingFileHelper);
    }
    ...
    private ItemModelBuilder complexBlock(Block block) {
        return withExistingParent(ForgeRegistries.BLOCKS.getKey(block).getPath(), new ResourceLocation(CrusadersProgressionReimagined.MOD_ID,
                "block/" + ForgeRegistries.BLOCKS.getKey(block).getPath()));
    }

    private void trimmedArmorItem(RegistryObject<Item> itemRegistryObject) {
        final String MOD_ID = CrusadersProgressionReimagined.MOD_ID; // Change this to your mod id
    ...
    private ItemModelBuilder simpleItem(RegistryObject<Item> item) {
        return withExistingParent(item.getId().getPath(),
                ...
                ResourceLocation.tryBuild(CrusadersProgressionReimagined.MOD_ID, "item/" + item.getId().getPath()));
    }
    private ItemModelBuilder handheldItem(RegistryObject<Item> item) {
        return withExistingParent(item.getId().getPath(),
                new ResourceLocation("item/handheld")).texture("layer0",
                new ResourceLocation(CrusadersProgressionReimagined.MOD_ID,"item/" + item.getId().getPath()));
    }
}
